...
The concrete, validated `Architecture` is the shared source model. It can
describe:
...
- native processor MLIR (registered templates are emitted in `frontend/`), and
...
MLIR export returns `AdlExportError::ValidatorUnavailable`. The unchecked
exporter is intended for deliberate debugging and experimental output, not as
...
The YAML preserves placed components, scopes, relationships, dimensions, and
replication metadata. Reusable definitions are folded into their placements;
memory aliases resolve connectivity without becoming storage nodes. Large
replicated structures remain compact metadata instead of expanding into one
node per hardware instance.
