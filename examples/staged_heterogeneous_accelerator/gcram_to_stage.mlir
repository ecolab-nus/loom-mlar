module @processor {
  func.func @load_gcram_f16(%src: memref<?x?xf16>, %dst: memref<?x?xf16, 2>) {
    ...
    loom.bind_mem %src, @gcram : memref<?x?xf16>
    loom.bind_mem %dst, @stage : memref<?x?xf16, 2>
    loom.copy %src, %dst src_mem_space @gcram dst_mem_space @stage, area: [1, 1] : memref<?x?xf16> to memref<?x?xf16, 2>
    return
  }
  func.func @load_gcram_f16_broadcast(%src: memref<?x?xf16>, %dst: memref<?x?xf16, 2>) {
    ...
    loom.bind_mem %src, @gcram : memref<?x?xf16>
    loom.bind_mem %dst, @stage : memref<?x?xf16, 2>
    loom.copy %src, %dst src_mem_space @gcram dst_mem_space @stage, area: [%bcst_x, %bcst_y] : memref<?x?xf16> to memref<?x?xf16, 2>
    return
  }
}
