...
use mlar_rust::arch::MemoryTechnology;
use mlar_rust::math::Sym;
use mlar_rust::mlir::{
...
pub fn lower_loom_source(
    ...
    for block in blocks {
        ...
        for (buffers, bindings, memories) in [
            (&parsed.inputs, &input_bindings, input_memories),
            (&parsed.outputs, &output_bindings, output_memories),
        ] {
            for memory in memories {
                for (buffer, binding) in buffers.iter().zip(bindings) {
                    if binding.symbol != memory.symbol {
                        continue;
                    }
                    output.push_str(&format!(
                        "    loom.bind_mem %{}, @{} : {}\n",
...
fn extent_items(line: &str) -> Result<Option<Vec<&str>>, LoomParseError> {
    ...
    let items = extent.split(',').map(str::trim).collect::<Vec<_>>();
...
fn parse_compact_function(block: &str) -> Result<CompactFunction, LoomParseError> {
    ...
    let mut explicit_parameters = Vec::new();
    struct BodyBlock {
        operation: String,
        depth: i64,
    ...
    for raw in body_text.lines() {
        ...
        if line.contains("loom.sym") {
            explicit_parameters.push(parse_symbol_declaration(line)?);
            continue;
        }
    ...
    let mut buffer_names = std::collections::BTreeSet::new();
    for buffer in inputs.iter().chain(&outputs) {
        if !buffer_names.insert(&buffer.name) {
            return Err(LoomParseError(format!(
                "function '{name}' has duplicate operand '{}'",
                buffer.name
            )));
        }
    }
    ...
    for parameter in explicit_parameters {
        if !declared.insert(parameter.clone()) {
            return Err(LoomParseError(format!(
                "function '{name}' has duplicate symbol '{parameter}'"
            )));
        }
        params.push(parameter);
    }
    mlar_rust::arch::perf::validate_symbols(&params.iter().map(Sym::new).collect::<Vec<_>>())
        .map_err(LoomParseError)?;
    for parameter in &params {
        if buffer_names.contains(parameter) {
            return Err(LoomParseError(format!(
                "function '{name}' symbol '{parameter}' conflicts with an operand"
            )));
        }
    }
    for operation in &body {
        ...
        for item in extent.into_iter().flatten() {
            if item.parse::<u64>().is_err() && !declared.contains(item) {
                return Err(LoomParseError(format!(
                    "function '{name}' extent uses undeclared symbol '{item}'; declare it with loom.sym"
                )));
            }
        }
    }
...
fn parse_symbol_declaration(line: &str) -> Result<String, LoomParseError> {
    let invalid = || LoomParseError(format!("expected `%name = loom.sym @name : index`: {line}"));
    let (ssa, declaration) = line.split_once('=').ok_or_else(invalid)?;
    let ssa = ssa.trim().strip_prefix('%').ok_or_else(invalid)?;
    let declaration = declaration
        .trim()
        .strip_prefix("loom.sym")
        .ok_or_else(invalid)?;
    let (symbol, ty) = declaration
        .trim()
        .strip_prefix('@')
        .ok_or_else(invalid)?
        .split_once(':')
    ...
    if ssa != symbol || ty.trim() != "index" {
        return Err(invalid());
    }
    mlar_rust::arch::perf::validate_symbols(&[Sym::new(symbol)]).map_err(LoomParseError)?;
    Ok(symbol.to_string())
}
...
fn parse_buffer(line: &str) -> Result<Buffer, LoomParseError> {
    ...
    mlar_rust::arch::perf::validate_symbols(&[Sym::new(name)]).map_err(LoomParseError)?;
...
"#;
    ...
    #[test]
    fn explicit_symbols_are_available_to_performance_and_lower_once() {
        let source = r#"
...
"#;
        ...
        assert_eq!(
            module.functions[0].symbols,
            Sym::from_names(["L", "effective_bandwidth"])
        );
        let perf = crate::PerformanceYaml::from_yaml_str(
            "copy:\n  - latency: '0'\n    volume: L * effective_bandwidth\n    throughput: '64'\n",
        )
        .unwrap();
        perf.model_for_func(&module.functions[0]).unwrap();
        let lowered = lower_loom_source(
            source,
            "copy",
            &Default::default(),
        ...
        assert_eq!(
            lowered
                .matches("%effective_bandwidth = loom.sym @effective_bandwidth : index")
                .count(),
            1
        );
        ...
        let module = parse_loom_source(&undeclared).unwrap();
        assert!(perf.model_for_func(&module.functions[0]).is_err());
    }

    #[test]
    fn rejects_invalid_symbol_declarations_and_extent_references() {
        for declaration in [
            "%L = loom.sym @L : index",
            "%src = loom.sym @src : index",
            "%x = loom.sym @y : index",
            "%x = loom.sym @x : i32",
            "%x = loom.sym @x : index\n%x = loom.sym @x : index",
        ] {
            ...
            assert!(parse_loom_source(&source).is_err(), "{declaration}");
        }
        for extent in ["X", "L + 1", "L,", "L,,L"] {
            let source = format!(
                "func @broadcast(in src: f16[L], out dst: f16[L]) {{\nloom.broadcast %src to %dst extent: [{extent}]\n}}"
            );
            assert!(parse_loom_source(&source).is_err(), "{extent}");
        }
        let source = "func @copy(in src: f16[L], out dst: f16[L]) {\nloom.copy %src to %dst\n}\n";
        assert!(parse_loom_source(&source.repeat(2)).is_err());
        assert!(parse_loom_source(&source.replace("out dst:", "out src:")).is_err());
        assert!(parse_loom_source(&source.replace("f16[L]", "f16[L,]")).is_err());
    }
...
fn resolve_operand_memory_bindings(
    function: &str,
    role: &str,
