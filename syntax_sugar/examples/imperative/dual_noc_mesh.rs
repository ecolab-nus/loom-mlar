...
pub fn build() -> ExampleResult<Architecture> {
    Ok(
        mlar_syntax_sugar::ArchitectureBuilder::new("dual_noc_system")
            .axis("dram_channel", 8)
            .axis("x", 8)
            .axis("y", 8)
            ...
            .connect(
                "dram_l1_noc0",
                Connection::parse([], ["DRAM[:]"], ["L1[:, :]"])?.with_resources(["noc0"]),
            )
            .connect(
                "l1_l1_noc0",
                Connection::parse([], ["L1[:, :]"], ["L1[:, :]"])?.with_resources(["noc0"]),
            )
            .connect(
                "l1_dram_noc1",
                Connection::parse([], ["L1[:, :]"], ["DRAM[:]"])?.with_resources(["noc1"]),
            )
            .build()?,
    )
}
