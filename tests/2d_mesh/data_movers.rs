...
pub fn dram_l1_mover() -> DataMover {
    let functionality =
        MlirModule::from_mlir("tests/2d_mesh/processors_mlir/dram_l1_mover.mlir")
    ...
    let perf_models: Vec<FuncPerfModel> =
        vec![unicast_perf_model.clone(), bcst_perf_model, unicast_perf_model]
            .into_iter()
            .map(|pm| {
    ...
    let dram = dram();
    let l1 = l1();
    let mut mover = DataMover::builder()
        .named("dram_l1_mover")
...
pub fn dram_l1_bcst_v_mover() -> DataMover {
    let functionality =
        MlirModule::from_mlir("tests/2d_mesh/processors_mlir/dram_l1_bcst_v.mlir")
...
pub fn dram_l1_bcst_h_mover() -> DataMover {
    let functionality =
        MlirModule::from_mlir("tests/2d_mesh/processors_mlir/dram_l1_bcst_h.mlir")
