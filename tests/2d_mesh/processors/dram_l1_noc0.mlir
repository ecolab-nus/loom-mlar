module @processor {
  func.func @dram_to_l1_S_f16(%dram_src: memref<?x?xf16>, %l1_dst: memref<?x?xf16>) {
    ...
    loom.bind_mem %l1_dst, @dst : memref<?x?xf16>
    loom.copy %dram_src, %l1_dst src_mem_space @src dst_mem_space @dst, area: [1, 1] : memref<?x?xf16> to memref<?x?xf16>
    return
  }
  func.func @dram_to_l1_S_bcst(%dram_src: memref<?x?xf16>, %l1_dst: memref<?x?xf16>) {
    ...
    loom.bind_mem %dram_src, @src : memref<?x?xf16>
    loom.bind_mem %l1_dst, @dst : memref<?x?xf16>
    loom.copy %dram_src, %l1_dst src_mem_space @src dst_mem_space @dst, area: [%bcst_x, %bcst_y] : memref<?x?xf16> to memref<?x?xf16>
    return
  }
  func.func @dram_to_l1_R_f16(%dram_src: memref<?x?xf16>, %l1_dst: memref<?x?xf16>) {
    ...
    loom.bind_mem %dram_src, @src : memref<?x?xf16>
    loom.bind_mem %l1_dst, @dst : memref<?x?xf16>
    loom.copy %dram_src, %l1_dst src_mem_space @src dst_mem_space @dst : 1, area: [1, 1] : memref<?x?xf16> to memref<?x?xf16>
    return
  }
  func.func @dram_to_l1_R_bcst(%dram_src: memref<?x?xf16>, %l1_dst: memref<?x?xf16>) {
    ...
    loom.bind_mem %dram_src, @src : memref<?x?xf16>
    loom.bind_mem %l1_dst, @dst : memref<?x?xf16>
    loom.copy %dram_src, %l1_dst src_mem_space @src dst_mem_space @dst : 1, area: [%bcst_x, %bcst_y] : memref<?x?xf16> to memref<?x?xf16>
    return
  }
}
