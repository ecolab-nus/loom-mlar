...
func.func @dram_to_l1_R_f16(
  ...
  loom.bind_mem %dram_src, @DRAM : memref<?x?xf16>
  loom.bind_mem %l1_dst, @array_L1 : memref<?x?xf16>
  loom.copy %dram_src, %l1_dst src_mem_space @DRAM dst_mem_space @array_L1 : 1, area: [1, 1] : memref<?x?xf16> to memref<?x?xf16>
  return
}
...
func.func @dram_to_l1_R_bcst(
  ...
  %bcst_x = loom.sym @bcst_x : index
  %bcst_y = loom.sym @bcst_y : index
  loom.copy %dram_src, %l1_dst src_mem_space @DRAM dst_mem_space @array_L1 : 1, area: [%bcst_x, %bcst_y] : memref<?x?xf16> to memref<?x?xf16>
  return
}
