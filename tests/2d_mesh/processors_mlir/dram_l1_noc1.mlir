...
// Writeback and gather each have rank-2 and batched rank-3 forms.
...
func.func @batch_l1_to_dram_f16(
  ...
  loom.bind_mem %l1_src, @array_L1 : memref<?x?x?xf16>
  loom.bind_mem %dram_dst, @DRAM : memref<?x?x?xf16>
  loom.copy %l1_src, %dram_dst src_mem_space @array_L1 dst_mem_space @DRAM, area: [1, 1] : memref<?x?x?xf16> to memref<?x?x?xf16>
  return
}
...
func.func @batch_l1_gather(
    ...
    %gather_x = loom.sym @gather_x : index
    %gather_y = loom.sym @gather_y : index
    loom.gather %l1_src, %l1_dst src_mem_space @array_L1 dst_mem_space @array_L1 area: [%gather_x, %gather_y] : memref<?x?x?xf16> to memref<?x?x?xf16>
    return
  }
}
