...
export function countBankLeaves(region: GraphMemoryRegion): number | null {
  switch (region.kind) {
    ...
    case 'replicated':
    case 'array': {
      const inner = countBankLeaves(region.sub_region);
      if (inner === null) {
        return null;
      }
...
function compactRegionSummary(region: GraphMemoryRegion): string {
  switch (region.kind) {
    ...
    case 'array': {
      ...
      const subRegionSize = regionTotalBytes(region.sub_region);
      if (n !== null && subRegionSize !== null) {
        return `${n} × ${formatBytesCompact(subRegionSize)}`;
      }
...
function regionTotalBytes(region: GraphMemoryRegion): number | null {
  ...
  switch (region.kind) {
    ...
    case 'array': {
      const subRegionSize = regionTotalBytes(region.sub_region);
      const multiplier = productConcreteSizes(region.dimensions);
      return subRegionSize !== null && multiplier !== null ? subRegionSize * multiplier : null;
    }
    case 'group': {
      let total = 0;
      for (const part of region.parts) {
